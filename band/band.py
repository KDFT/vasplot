...
#   else:
    if fermi_range[0] != fermi_range[1]:
      for i in range(fermi_range[0], fermi_range[1]):
        print("fermi {0:6.3f} ~ {1:6.3f}".format(bandmin[i],bandmax[i]))
    i = fermi_range[1]
...
# procar[k][b][i][o]
  ...
  def plotfatband(self, ax, Kdist, norm, plot_mode):
# Plot fat band
    nbands = len(Band.eigen)
    line = []
    for e, f in list(zip(Band.eigen, self.fat)):
      if plot_mode == 'fat' : line = self.plotfillband(ax, Kdist, e, normalize(f, norm))
      if plot_mode == 'line': self.plotlineband(ax, Kdist, e, normalize(f, norm))
#     line = self.plotfillband(ax, Kdist, e, normalize(f, norm))
    if Band.ispin == 2:
      for e, f in list(zip(Band.eigendn, self.fatdn)):
        if plot_mode == 'fat' : self.plotfillband(ax, Kdist, e, normalize(f, norm))
        if plot_mode == 'line': self.plotlineband(ax, Kdist, e, normalize(f, norm))
#       self.plotfillband(ax, Kdist, e, normalize(f, norm))
  ...
  def plotlineband(self, ax, Kdist, eigen, fat):
    import numpy as np
    ...
    for i in range(N):
      temp = []
      for j in range(3):
        temp.append((blue[j]*(N-i)+green[j]*i)/N)
      temp.append(1.)
      vals.append(temp)
    for i in range(N):
      temp = []
      for j in range(3):
        temp.append((green[j]*(N-i)+red[j]*i)/N)
      temp.append(1.)
      vals.append(temp)
    ...
    segments = []
    old = [Kdist[0], eigen[0]]
    for k, e in list(zip(Kdist[1:], eigen[1:])):
      segments.append([old, [k,e]])
      old = [k,e]

    lc = LineCollection(segments, cmap=cmap, norm=plt.Normalize(0., self.fmax/2.5))
#   lc = LineCollection(segments, cmap=cmap, norm=plt.Normalize(0., self.fmax))
#   lc = LineCollection(segments, cmap=cmap, norm=plt.Normalize(0., self.fmax/6.))
    lc.set_array(np.array(fat))
    lc.set_linewidth(2.)
    ax.add_collection(lc)
    if FatBand.axcb == None: FatBand.axcb = plt.colorbar(lc)
